...
class LSHash(object):
    ...
    def _hash(self, planes, input_point):
        ...
        try:
            ...
            print(
                """The input point needs to be an array-like object with
            ...
            print(
                """The input point needs to be of the same dimension as
                  `input_dim` when initializing this LSHash instance""",
                e,
            )
            raise
        else:
    ...
    def save(self):
        ...
        if self.hashtable_filename:
            try:
                np.savez_compressed(
                    self.hashtable_filename, allow_pickle=True, data=self.hash_tables
                )
    ...
    def query(self, query_point, num_results=None, distance_func: str = "euclidean"):
        ...
        candidates = [
            (ix, d_func(query_point, self._as_np_array(ix))) for ix in candidates
        ]
